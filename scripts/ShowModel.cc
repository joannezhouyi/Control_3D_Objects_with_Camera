...
void vMotion(int iSX, int iSY, int iDX, int iDY, int iFlag)
{
    ivec2 PosStart(iSX, iSY);
    ivec2 PosDiff(iDX, iDY);
    if(iFlag == 0)
    {
        curRot = PosStart + PosDiff;

        //gfScaling  += (float)( x - y ) / 100.0f;
    }
    else if(iFlag == 1)
    {
        curTrans = PosStart + PosDiff;
    }
    glutPostRedisplay();
}
...
void display()
{
    ...
    // apply zoom in/out
    glScalef(gfScaling, gfScaling, gfScaling);
...
struct stShowModel
{
    ...
    void vCameraControl(int iSX, int iSY, int iDX, int iDY, int iSteadyFlag)
    {
        int iFlagRot = 0;
        if(iSteadyFlag == 1)
        {
        ...
        if(iSteadyReady == 1)
        {
            vMotion(iSX, iSY, iDX, iDY, iFlagRot);
        }
    }
    int iSteadyReady = 0;
};
